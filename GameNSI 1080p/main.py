...
x = 0
c = 0
font = pygame.font.Font('assets/corbel-light.ttf', 100)


def show_score():
    score = font.render(str(c), True, (0, 0, 0))
    screen.blit(score, (1800, 10))
...
while running:
    ...
    while playing:
        ...
        player1.all_coins.update(screen)
        show_score()
        pygame.display.update()
        clock.tick(60)
        for event in pygame.event.get():
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_ESCAPE:
                    playing = False
                    menu = True
        if player1.Collision(player1, player1.all_rp):
            for i in range(0, 1):
                player1.spawn_redplatforms()
            c = 0
            playing = False
            menu = True
        if player1.Collision(player1, player1.all_coins):
            for i in range(0, 1):
                player1.spawn_coins()
            c += 1
